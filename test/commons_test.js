...
exports['Commons Test'] = {
	...
	'validation': {
		'valid monolith mode': function( test ){
			...
			test.ok( commons.validateMeta( meta, errors ), 'it should pass the validation' );
		...
		'invalid META': function( test ){
			...
			test.ok( !commons.validateMeta( meta, errors ), 'it should not pass the validation' );
			test.strictEqual( errors.toString(), expected, 'it should return the same errors' );
			test.done();
		}
	} // validation
};
