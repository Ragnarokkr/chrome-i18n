  ...
  grunt.registerTask( 'bump', 'Bump the version number', function( release ){
    ...
    if ( arguments.length && /^major|minor|patch|build$/.test( release ) ) {
      ...
      if ( newVersion ) {
        ...
        grunt.log.writeln( this.name + ': version bumped to ' + newVersion + ' (it was ' + oldVersion + ')' );
        return true;
      } // if
    }

    // executed ONLY if no argument is passed
    grunt.fail.warn( this.name + ': please specify what release to bump: major, minor, patch, or build' );
    return false;
  });
};
